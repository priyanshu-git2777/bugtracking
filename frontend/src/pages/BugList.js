    ...
    <div style={{ padding:'2rem', maxWidth:1300, margin:'0 auto' }}>
      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', flexWrap:'wrap', gap:'1rem', marginBottom:'1.5rem' }}>
        ...
        <div style={{ display:'flex', gap:'.75rem', flexWrap:'wrap', alignItems:'center' }}>
          ...
          <button onClick={()=>nav('/create-bug')} style={{ padding:'.5rem 1.2rem', background:'#ff4d6d', color:'#fff', fontSize:'.85rem' }}>
            + Report Bug
          </button>
        </div>
      </div>
        ...
        {filtered.map(b => (
          <div key={b._id} style={{ display:'grid', gridTemplateColumns:'2.5fr 1fr 1fr 1fr 1.5fr 1fr', gap:'1rem', padding:'14px 1.5rem', borderTop:'1px solid #1e2330', fontSize:'.875rem', alignItems:'center' }}>
            ...
            <span style={{ display:'flex', gap:6 }}>
              <button onClick={()=>openEdit(b)} style={{ background:'#191c25', color:'#e8eaf0', padding:'4px 10px', border:'1px solid #1e2330', fontSize:'.75rem' }}>Edit</button>
